...
import { customizeFunnel, FunnelWithExplanationSchema } from '../src/tools/funnel.ts';

const SKIP_OPENAI = !Deno.env.get('RUN_OPENAI_TESTS');

Deno.test({
	name: 'customizeFunnel - successful OpenAI call',
	ignore: SKIP_OPENAI,
	async fn() {
...
Deno.test({
	name: 'customizeFunnel - customizes for specific sector',
	ignore: SKIP_OPENAI,
	async fn() {
		const result = await customizeFunnel(
			'healthcare',
...
Deno.test({
	name: 'customizeFunnel - handles different languages',
	ignore: SKIP_OPENAI,
	async fn() {
		const result = await customizeFunnel(
			'technology',
...
Deno.test({
	name: 'customizeFunnel - fails with invalid API key',
	ignore: SKIP_OPENAI,
	async fn() {
...
Deno.test({
	name: 'customizeFunnel - preserves funnel stages',
	ignore: SKIP_OPENAI,
	async fn() {
		const result = await customizeFunnel(
			'e-commerce',
